...
struct NativeDownloadsScreen: View {
    ...
    private func size(_ item: OfflineDownload) -> String {
        guard let url = item.localURL, let enumerator = FileManager.default.enumerator(at: url, includingPropertiesForKeys: [.fileSizeKey]) else { return "Boyut hesaplanıyor" }
